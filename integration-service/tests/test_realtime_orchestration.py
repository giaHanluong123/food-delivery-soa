...
def test_create_order_flow_publish_realtime(monkeypatch):
    http_calls = []
    published_events = []

    def fake_request(self, method, url, json=None):
        http_calls.append({"method": method, "url": url, "json": json})
        ...
        if url == f"{DELIVERY_SERVICE_URL}/deliveries":
            return {
                "id": 301,
                "order_id": 101,
                "user_id": 1,
                ...
                "dropoff_address": "123 Le Loi, Ben Nghe, District 1, Ho Chi Minh City",
                "shipping_fee": 15000,
                "note": "Test order",
            }
    ...
    monkeypatch.setattr(RealtimeEventService, "publish_event", fake_publish_event)
    ...
    assert published_events[0]["event_type"] == "order_created"
    assert published_events[0]["order_id"] == "101"
    assert published_events[0]["status"] == "pending"


def test_payment_callback_paid_publish_realtime(monkeypatch):
    published_events = []
    ...
    assert published_events[0]["event_type"] == "payment_paid"
    assert published_events[1]["event_type"] == "order_confirmed"


def test_delivery_delivered_publish_realtime(monkeypatch):
    published_events = []
    ...
    assert len(published_events) == 2
    assert published_events[0]["event_type"] == "delivery_delivered"
    assert published_events[1]["event_type"] == "order_delivered"